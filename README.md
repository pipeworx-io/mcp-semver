...
`tools/list` at `https://gateway.pipeworx.io/semver/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/parse_semver \
  -H 'Content-Type: application/json' \
  -d '{"version":"1.2.3"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/parse_semver`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
